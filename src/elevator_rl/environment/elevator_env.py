...
class ElevatorEnv:
    ...
    def render(
        ...
        action: ElevatorEnvAction = None,
    ):
        render(self.house, method, path, prev_time, action)
    ...
    def to_hashable(self) -> Tuple:
        ...
        """
        return tuple(
            (x.elevator_idx, x.elevator_action.value) for x in self._action_history
        )
    ...
    def _get_pending_requests_penalty(self):
        ...
        for i, present in enumerate(self.house.up_requests):
            if present:
                pending_requests += (
                    self.house.time - self.house.up_requests_waiting_since[i]
                ) ** 2
        for i, present in enumerate(self.house.down_requests):
            if present:
                pending_requests += (
                    self.house.time - self.house.down_requests_waiting_since[i]
                ) ** 2
