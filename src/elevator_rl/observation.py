...
class Observation:
    ...
    def __init__(self, house: House, next_elevator_idx: int):
        ...
        for elevator_idx, elevator in enumerate(house.elevators):
            passengers_waiting_time = [passenger.waiting_since for passenger in elevator.passengers]
            elevator_obs = np.array([elevator.floor,
                                     passengers_waiting_time,
                                     len(passengers_waiting_time),
    ...
    def as_array(self) -> np.ndarray:
        temp = np.array([self.number_of_floors,
