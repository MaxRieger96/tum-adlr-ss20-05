...
class CollectiveControl:
    ...
    def latest_direction_is(self, env: ElevatorEnv, direction: ElevatorActionEnum):
        return env.next_elevator in self.elevators_latest_direction and \
               self.elevators_latest_direction[
                   env.next_elevator] == direction
    ...
    def get_action(self, env: ElevatorEnv):
        ...
        if elevator.floor == env.house.number_of_floors - 1:
            ...
            house_requests_above = np.any(env.house.up_requests[elevator.floor + 1:]) or np.any(
                env.house.down_requests[elevator.floor + 1:])
        house_requests_below = np.any(env.house.up_requests[:elevator.floor]) or np.any(
            env.house.down_requests[:elevator.floor])

        if floor_requests_current:  # Passengers want to leave
            return ElevatorActionEnum.OPEN
        elif env.house.up_requests[elevator.floor] and \
                self.latest_direction_is(env, ElevatorActionEnum.UP):  # Pick up on way up
            return ElevatorActionEnum.OPEN
        elif env.house.down_requests[elevator.floor] and \
                self.latest_direction_is(env, ElevatorActionEnum.DOWN):  # Pick up on way down
            return ElevatorActionEnum.OPEN
        elif self.latest_direction_is(env, ElevatorActionEnum.UP) and (
                floor_requests_above or house_requests_above):  # More to do at the top
            ...
            return ElevatorActionEnum.UP
                ...
                floor_requests_below or house_requests_below):  # More to do at the bottom
            self.elevators_latest_direction[env.next_elevator] = ElevatorActionEnum.DOWN
            return ElevatorActionEnum.DOWN
        # COLD START SCENARIOS
        elif floor_requests_below:  # first person gets in at the floor the elevator already is at
            self.elevators_latest_direction[env.next_elevator] = ElevatorActionEnum.DOWN
            return ElevatorActionEnum.DOWN
        elif floor_requests_above:  # first person gets in at the floor the elevator already is at
            self.elevators_latest_direction[env.next_elevator] = ElevatorActionEnum.UP
            return ElevatorActionEnum.UP
